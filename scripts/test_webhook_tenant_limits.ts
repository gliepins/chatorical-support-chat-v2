...
// Mock prisma/channel for secret/tenant and settings
(globalThis as any).__prisma = {
  ...
  setting: {
    findUnique: async ({ where }: any) => {
      if (where.tenantId_key.key === 'rl.telegram_webhook.points') return { value: '1' };
      if (where.tenantId_key.key === 'rl.telegram_webhook.durationSec') return { value: '60' };
      if (where.tenantId_key.key === 'flags.telegram.disableInbound') return { value: 'true' };
      return null;
    }
  },
};
...
async function main() {
  ...
  // First request with correct header should be rate-limited to 1/minute and disabled inbound should still return ok
  ...
  const status429: number = await new Promise((resolve, reject) => {
    const req = http.request(`${base}/v1/telegram/webhook/whsec`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-telegram-bot-api-secret-token': 'HS' } }, (res:any) => { res.on('data',()=>{}); res.on('end', ()=> resolve(res.statusCode)); });
    req.on('error', reject); req.write(JSON.stringify({ ...update, update_id: 2 })); req.end();
  });
  await new Promise((r)=>server.close(()=>r(null)));
  if (status429 !== 429) { console.error('expected 429 on second webhook'); process.exit(1); }
  console.log('OK webhook tenant limits');
  process.exit(0);
}
