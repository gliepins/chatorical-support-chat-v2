...
const wsOutboundMessagesByTenant = new Map<string, number>();
...
const overLimitByTenantBucket = new Map<string, number>();
...
export function incWsOutboundForTenant(tenantId: string, count = 1) {
  if (!tenantId) return;
...
export function recordTelegramWebhookLatency(ms: number) {
...
export function incOverLimitForTenant(bucketName: string, tenantId: string) {
  overLimitTotal += 1;
  const key = `${tenantId}|${bucketName}`;
